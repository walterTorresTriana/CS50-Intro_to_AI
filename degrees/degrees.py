...
def shortest_path(source, target):
    ...
    source_start = Node(state=source, parent=None, action=None)
    frontier = QueueFrontier()
    frontier.add(source_start)
    
    # Set to track explored nodes
    explored_nodes = set()

    # Dictionary to track the path (movie_id, person_id)
    path = []
    ...
    while not frontier.empty():
        ...
        for movie_id, person_id in neighbors_for_person(current_node.state):
            if person_id not in explored_nodes and not frontier.contains_state(person_id):
